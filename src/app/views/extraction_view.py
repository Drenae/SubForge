...
class ExtractionView(ft.Container):
    def __init__(self, state: MediaState):
        ...
        super().__init__(
            ...
            content=ft.Column(expand=True, spacing=16, controls=[
                ft.Text("Extraction", size=30, weight=ft.FontWeight.BOLD, color=theme.TEXT),
                ft.Text("Extrait les pistes cochées dans Accueil dans leur format d'origine, sans conversion.",
    ...
    async def _extract(self, _):
        ...
        if not jobs or self.destination is None:
            self.summary.value = "Sélectionnez des pistes dans Accueil et un dossier de sortie."
            self._safe_update()
            return
        self.running = True
        self.cancel_requested = False
